...
# {'old_action': '', 'action': 'b200', 'client_pos': 0, 'hole_cards': ['Ac', '9d'], 'board': [],
# 'token': 'a2f42f44-7ff6-40dd-906b-4c2f03fcee57'}
#
...
#
# A client_pos of 0 indicates that you are the big blind (second to act pre-flop, first to act
# post-flop).  1 indicates you are the small blind.
#
...
# pre-flop action.  If the bet is called, then each player will have put a total of 600 chips into
# the pot counting both the pre-flop and the flop.
# 
...
def play_hand(token, game_env, client_pos=0):  # client_pos = first_player
    r = new_hand(token)
    ...
    while True:
        ...
        if winnings is not None:
            ...
            return token, winnings  # the current hand ends here
        ...
        if a['last_bettor'] == -1:
            ...
            # ^^ to also account for any calls that would have been missed otherwise

            # if player_n_bets[client_pos] == max(player_n_bets.values()) and player_n_bets[client_pos] < STACK_SIZE:  # should not reach here
        ...
        if CLIENT_TYPE == 0:
            ...
            if DISPLAY_INFO:
                print('INFO::Slumbot\'s last action: %s' % NUM2ACTION[his_last_action[0]])
        ...
        if DISPLAY_INFO:
            if CLIENT_TYPE == 0:
                ...
                print('INFO::Slumbot\'s last action: %s | Client\'s action: %s | player&bets: %s' % (
                    NUM2ACTION[his_last_action[0]], my_action, str(player_n_bets)))
    ...
    # Should never get here --> end of `play_hand()`
...
def main():
    ...
    for h in range(num_hands):
        env.card_play_init_no_data() # modified to *not* use `card_play_data`

        print('\n>>>Starting hand #%i/%i...' % ((h + 1), num_hands))
        (token, hand_winnings) = play_hand(token, env, client_pos=(h % 2))
        
        winnings += hand_winnings
        env.reset()

    bb100 = round(winnings / BIG_BLIND / num_hands * 100)
    print('Total winnings: %i, or %s BB/100, after %i games' % (winnings, ('\033[91m' if bb100 < 0 else '\033[92m') + str(bb100) + '\033[0m', num_hands))
